...
const HomePage = async () => {
  ...
  return (
    <main className="mx-auto flex min-h-dvh w-full max-w-3xl flex-col justify-center px-6 py-16">
      <p className="text-subheading-xs text-text-soft-400 mb-4 uppercase">{dict.eyebrow}</p>

      <h1 className="text-title-h3 text-text-strong-950 max-w-2xl text-balance">{dict.title}</h1>
      ...
      <p className="text-paragraph-lg text-text-sub-600 mt-4 max-w-xl">
        <strong className="text-text-strong-950 font-medium">{dict.descriptionLead}</strong>{' '}
        {dict.description}
      </p>
      ...
      <div className="mt-8 flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4">
        ...
        <span className="text-paragraph-xs text-text-soft-400">{dict.ctaHint}</span>
      </div>
      ...
      <ul className="border-stroke-soft-200 mt-14 grid gap-8 border-t pt-10 sm:grid-cols-3">
        {dict.features.map(feature => (
          <li key={feature.title} className="flex flex-col gap-1.5">
            <h2 className="text-label-md text-text-strong-950">{feature.title}</h2>
      ...
      <p className="text-paragraph-xs text-text-soft-400 mt-12 max-w-2xl">{dict.dataNote}</p>
    </main>
  );
};
