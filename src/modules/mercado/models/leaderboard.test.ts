...
describe('construirFilas', () => {
  it('marca la fila del usuario y ninguna otra', () => {
    ...
    assert.deepEqual(filas.filter(f => f.esMio).map(f => f.provider), ['BIA ENERGY']);
  });
  ...
  it('calcula el delta contra MI tarifa, con el signo correcto', () => {
    ...
    assert.ok(Math.abs((enerbit?.deltaPorcentual ?? 0) - -2.66) < 0.05, `delta inesperado: ${enerbit?.deltaPorcentual}`);
  ...
  it('sin empresa registrada tampoco inventa deltas', () => {
    assert.ok(construirFilas(CARIBEMAR, null).every(f => f.deltaPorcentual === null));
    assert.ok(construirFilas(CARIBEMAR, undefined).every(f => f.deltaPorcentual === null));
  });
  ...
  it('un CU en cero no produce Infinity', () => {
    const filas = construirFilas([agente('YO', 0, 1), agente('OTRO', 500, 2)], 'YO');
    assert.ok(filas.every(f => f.deltaPorcentual === null));
  });

  it('conserva el orden que trae el backend', () => {
    const filas = construirFilas(CARIBEMAR, 'BIA ENERGY');
    assert.deepEqual(filas.map(f => f.provider), ['ENERBIT', 'BIA ENERGY', 'ENERTOTAL', 'AFINIA']);
  });
});
